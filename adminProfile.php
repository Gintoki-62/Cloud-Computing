...
<!----------------------------------------- get detail ----------------------------------->    
    ...
    else
    {
       ...
       $error = validationwithoutid();
        ...
        {
            //step 2
             $sql = "UPDATE users SET username=?,phone=?,email=?,address=?,photo=? WHERE user_id=?";
             
            //step 3
             $stmt = $conn->prepare($sql);
             
             $stmt->bind_param('sssssd',$username, $phone, $email, $address, $profile_photo, $user_id);
...
<br/>
    <form action="" method="POST" enctype="multipart/form-data" style="float:right; border: 2px solid gray; box-shadow: 3px 3px 3px grey ;
        ...
        <tr>
            <td>Address &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;:</td>
            <td><input class="search1" type="text" name="address" value="<?php echo isset($address)?$address:"" ?>" style="width: 550px;padding:5px; border-radius: 6px; border: none;background-color:transparent;" readonly/></td>
        </tr>
    </table>
        <br/>
        <div style="text-align: center">
                <input class="cart-btn" type="button" value="EDIT" onclick="window.location.href='editProfile.php?user_id=<?php echo $user_id ?>'"/> &nbsp;&nbsp;&nbsp;
                <input class="cart-btn" type="button" value="BACK" onclick="window.location.href='homepage.php'"/>
            </div>       
    </form>  
